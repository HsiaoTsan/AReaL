...
class TestPPOActorLossFnSequenceLevel:
    ...
    def test_sequence_level_geometric_mean_1d(self):
        ...
        assert torch.allclose(
            ...
            expected_ratio.expand(total_tokens),
            atol=1e-5
        )
    ...
    def test_sequence_level_with_mask_2d(self):
        ...
        assert torch.allclose(
            stat["importance_weight"][0, 4:],
            torch.zeros(4),
            atol=1e-6
        )
        assert torch.allclose(
            stat["importance_weight"][1, 6:],
            torch.zeros(2),
            atol=1e-6
        )
        ...
        assert torch.allclose(
            stat["importance_weight"][0, 0],
            stat["importance_weight"][0, 3],
            atol=1e-5
        )
    ...
    def test_sequence_level_vs_token_level_different(self):
        ...
        assert not torch.allclose(
            stat_seq["importance_weight"],
            stat_tok["importance_weight"],
            atol=1e-6
        )
...
class TestPPOActorLossFnEdgeCases:
    ...
    def test_batch_size_1_with_partial_mask(self):
        ...
        loss_mask = torch.tensor([[True, True, True, True, True, True, True, False, False, False]])
